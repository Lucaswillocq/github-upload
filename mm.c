...
Block* searchList(size_t reqSize) {
  ...
  {
      if(checkSize>=(ptrFreeBlock->info.size))
        {return ptrFreeBlock;}
      else
...
void* mm_malloc(size_t size) {
  ...
  size_t blockinfoSize = sizeof(ptrFreeBlock->info);

  if(blockinfoSize+sizeof(Block)<reqSize)
  {
    splitBlock = (Block*)UNSCALED_POINTER_ADD(ptrFreeBlock,sizeof(Block)+reqSize);
...
void mm_free(void* ptr) {
  //Block* FreeBlockInfo = (Block*)UNSCALED_POINTER_SUB(ptr, sizeof(BlockInfo));
  Block* ptrFreeBlock = ptr;
  ptrFreeBlock->info.size = -(ptrFreeBlock->info.size);
